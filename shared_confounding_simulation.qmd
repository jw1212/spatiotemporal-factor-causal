...
run_estimators <- function(s, d, y, m_fit, dreg, yreg) {
  ...
  for (model in c("stack_dml", "nc_mfactor", "ife")) {
    est <- fit_spatial_invariant(model, s, d, y, m_fit, dml_rf)
...
label_models <- function(df) {
  df %>%
    ...
    mutate(model = factor(model, levels = c("NUC", "IFE", "FC")))
}
...
run_nonGaussian_sim <- function(
  ...
  for (i in seq_len(n_simulations)) {
    ...
    for (ftype in dist_types) {
      ...
      fit_df <- run_estimators(s, d, y, m_fit = m_fit, dreg = dreg, yreg = yreg)
      fit_df$simulation <- i
      fit_df$facet <- ftype
...
run_nonlinear_sim <- function(
    ...
    both_pairs = list(
      c("linear_plus_cubic", "linear_plus_cubic"),
      c("cubic", "sine"),
      c("tanh", "tanh")
    ),
    m_fit = m,
...
# ggsave("sim_nonGaussian_N30T100.pdf", p_nonGaussian,
#        width = 8, height = 5.5, units = "in", dpi = 300)

# ggsave("sim_nonlinear_N30T100.pdf", p_nonlinear,
#        width = 8, height = 5.5, units = "in", dpi = 300)
```
...
# saveRDS(results_rank_fc_df, file = "sim_rank_fc_N30T100.rds")
# saveRDS(selected_rank_tab, file = "sim_rank_sel_N30T100.rds")

# -----------------------------
# Main figure: estimation error by true rank
# -----------------------------
p_rank_fc <- ggplot(results_rank_fc_df[results_rank_fc_df$true_rank != 15,],
...
print(p_rank_fc)
# ggsave("sim_rank_fc_N30T100.pdf", p_rank_fc, width = 8, height = 5, units = "in", dpi = 300)

-----------------------------
# Supplementary figure: selected rank frequencies
# -----------------------------
p_rank_sel <- ggplot(selected_rank_tab[selected_rank_tab$true_rank != 15,],
...
print(p_rank_sel)
# ggsave("sim_rank_sel_N30T100.pdf", p_rank_sel, width = 8, height = 5, units = "in", dpi = 300)
```
