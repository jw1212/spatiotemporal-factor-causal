...
run_nonlinear_sim <- function(
  ...
  for (i in seq_len(n_simulations)) {
    ...
    lin_D <- u %*% t(A)      # BU-type exposure index
    lin_Y <- u %*% t(gamma)  # Gamma U-type confounding index
    ...
    for (etype in exposure_types) {
      ...
      fit_df <- run_estimators(s, d, y, m_fit = m_fit, dreg = dreg, yreg = yreg)
      fit_df$simulation <- i
      fit_df$facet <- paste0("Exp: ", case_when(
        etype == "linear_plus_cubic" ~ "Lin + Cubic",
        etype == "cubic" ~ "Cubic",
        etype == "tanh" ~ "Tanh"
      ))
    ...
    for (ytype in outcome_types) {
      conf_signal <- apply_nonlinearity(lin_Y, ytype)
      d <- lin_D + epsD
      y <- d %*% t(B_true) + conf_signal + epsY

      fit_df <- run_estimators(s, d, y, m_fit = m_fit, dreg = dreg, yreg = yreg)
      fit_df$simulation <- i
      fit_df$facet <- paste0("Out: ", case_when(
        ytype == "linear_plus_cubic" ~ "Lin + Cubic",
        ytype == "sine" ~ "Sine",
        ytype == "tanh" ~ "Tanh"
      ))
  ...
  bind_rows(results) %>%
    ...
    mutate(
      facet = factor(
        facet,
        levels = c(
          "Exp: Lin + Cubic", "Exp: Cubic", "Exp: Tanh",
          "Out: Lin + Cubic", "Out: Sine", "Out: Tanh"
        )
      )
    )
}
