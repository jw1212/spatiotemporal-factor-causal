  ...
  for (ftype in facet_types) {
    ...
    if (ftype %in% struct_types) {
      ...
      if (ftype == "discreteU") {
        ...
        if (ftype == "nonlinearD") {
          lin_nl <- lin^3
          lin_nl <- scale(lin_nl, center = TRUE, scale = TRUE)
          d <- lin_nl + epsD
          y <- (d %*% t(B_true)) + (u %*% t(gamma)) + epsY

        } else if (ftype == "nonlinearY") {
          conf_lin <- u %*% t(gamma)            # t x N
          conf_nl <- sin(conf_lin)
          conf_nl <- scale(conf_nl, center = TRUE, scale = TRUE)

          d <- lin + epsD
          y <- (d %*% t(B_true)) + conf_nl + epsY
        }
      }
    }
...
p8 <- ggplot(results_non_Gaussian_df,
  ...
  coord_cartesian(ylim = c(0.94, 1.4))
